...
- Codex API (core 논문 요약 전용), OpenAI Embeddings (text-embedding-3-small, 1536d)
...
### 설계 원칙
- LLM은 core 논문 요약에만 사용. 나머지는 전부 코드 기반.
- 별도 온톨로지 없음. 기존 논문 데이터 자체가 연구자 프로필.
- 키워드 빈도, 저널 빈도는 실시간 DB 집계. 별도 저장 안 함.
